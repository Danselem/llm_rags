...
def paper_analysis_on_laptop (model_name, from_hf=False):

    # Load the llmware sample files
    print (f"\n > Loading the llmware sample files...")
    sample_files_path = Setup().load_sample_files()
    papers_path = os.path.join(sample_files_path,"Radiopapers")
 
    # query list
    query_list = {"paper title": "What is the title of the paper?",
                  "abstract summary": "Summarize the abstract?",
                  "keywords": "What the the keywords in the paper?"}
    ...
    for i, paper in enumerate(os.listdir(papers_path)):
        
        if paper != ".DS_Store":

            print("\nAnalyzing contract: ", str(i+1), paper)

            print("LLM Responses:")
            for key, value in query_list.items():

                # contract is parsed, text-chunked, and then filtered by topic key
                source = prompter.add_source_document(papers_path, paper, query=key)
...
if __name__ == "__main__":
    ...
    paper_analysis_on_laptop(model)
